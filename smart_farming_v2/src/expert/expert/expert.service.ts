...
async getMessages(senderId: number, receiverId: number): Promise<Message[]> {
  return this.messageRepo.find({
    ...
    order: { timestamp: 'ASC' }, // Or 'DESC' depending on your requirements
  });
}

async receivePayment(expertId: number, paymentDto: PaymentDto): Promise<any> {
  // Here you would typically interact with your payment gateway or service
  // and handle the payment processing logic.

  // For this example, let's assume a successful payment returns a receipt.
  const receipt = `Payment of $${paymentDto.amount} received via ${paymentDto.paymentMethod}.`;

  // Update expert's balance and other payment-related information
  const expert = await this.expertRepo.findOneBy({ id: expertId });
  if (!expert) {
    throw new NotFoundException(`Expert with ID ${expertId} not found`);
  }

  // Update expert's balance and other payment-related fields based on paymentDto
